...
namespace ModCreator.Models
{
    public class RowElementBinding : AutoNotifiableObject
    {
        ...
        private void ProcessElementFormat()
        {
            ...
            foreach (var element in _allElements.Where(e => !string.IsNullOrEmpty(e.ElementFormat)))
            {
                if (element.IsAutoGenerated)
                {
                    var generatedValue = PatternHelper.ProcessAutoGenValue(element.ElementFormat, _row);
                    if (_row.ContainsKey(element.Name))
